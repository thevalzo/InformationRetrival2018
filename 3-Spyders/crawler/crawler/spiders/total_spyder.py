...
class repubblicaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if(source==18935802): #Repubblica
            list = soup.findAll("span", {"itemprop": "articleBody"}) #testo di articolo
            if (not list):
                list = soup.findAll("p", {"class": "wp-caption-text"}) #descrizione di video
            if (not list):
                list = soup.findAll("span", {"itemprop": "description"}) #descrizione di immagine
